...
class GmailSendToolInput(BaseModel):
    ...
    sendTo: Optional[str] = Field(None, description="The email addresses of the recipients. Multiple addresses can be separated by a comma. e.g. jay@getsby.com, jon@smith.com.")
...
class GmailSendTool(BaseTool):
    name = "gmail_send"
    description = "Tool for gmail send operation - send operation"
    
    def _run(self, **kwargs):
        """Run the gmail send operation."""
        # Implement the tool logic here
        return f"Running gmail send operation with args: {kwargs}"
    
    async def _arun(self, **kwargs):
        """Run the gmail send operation asynchronously."""
