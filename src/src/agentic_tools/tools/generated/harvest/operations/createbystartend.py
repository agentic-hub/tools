...
class HarvestCreatebystartendToolInput(BaseModel):
    ...
    spentDate: Optional[str] = Field(None, description="The ISO 8601 formatted date the time entry was spent")
    ...
    projectId: Optional[str] = Field(None, description="The ID of the project to associate with the time entry")
    authentication: Optional[str] = Field(None, description="Authentication")


class HarvestCreatebystartendTool(BaseTool):
    name = "harvest_createbystartend"
    description = "Tool for harvest createByStartEnd operation - createByStartEnd operation"
    
    def _run(self, **kwargs):
        """Run the harvest createByStartEnd operation."""
        # Implement the tool logic here
        return f"Running harvest createByStartEnd operation with args: {kwargs}"
    
    async def _arun(self, **kwargs):
        """Run the harvest createByStartEnd operation asynchronously."""
