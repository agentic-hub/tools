...
class WekanUpdateToolInput(BaseModel):
    ...
    checklistItemId: Optional[str] = Field(None, description="The ID of the checklistItem item to update. Choose from the list, or specify an ID using an <a href=\"https://docs.n8n.io/code-examples/expressions/\">expression</a>.")
    checklistId: Optional[str] = Field(None, description="The ID of the checklistItem that card belongs to. Choose from the list, or specify an ID using an <a href=\"https://docs.n8n.io/code-examples/expressions/\">expression</a>.")
    ...
    cardId: Optional[str] = Field(None, description="The ID of the card to update. Choose from the list, or specify an ID using an <a href=\"https://docs.n8n.io/code-examples/expressions/\">expression</a>.")
    title: Optional[str] = Field(None, description="The title of the board")


class WekanUpdateTool(BaseTool):
    name = "wekan_update"
    description = "Tool for wekan update operation - update operation"
    
    def _run(self, **kwargs):
        """Run the wekan update operation."""
        # Implement the tool logic here
        return f"Running wekan update operation with args: {kwargs}"
    
    async def _arun(self, **kwargs):
        """Run the wekan update operation asynchronously."""
