...
class ThehiveprojectExecuteanalyzerToolInput(BaseModel):
    ...
    data_type: Optional[str] = Field(None, description="Type of the observable. Choose from the list, or specify an ID using an <a href=\"https://docs.n8n.io/code-examples/expressions/\">expression</a>.")
...
class ThehiveprojectExecuteanalyzerTool(BaseTool):
    name = "thehiveproject_executeanalyzer"
    description = "Tool for theHiveProject executeAnalyzer operation - executeAnalyzer operation"
    ...
    def _run(self, **kwargs):
        """Run the theHiveProject executeAnalyzer operation."""
        ...
        if credentials:
            ...
            return f"Running theHiveProject executeAnalyzer operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running theHiveProject executeAnalyzer operation with default credentials and args: {kwargs}"
    
    async def _arun(self, **kwargs):
        """Run the theHiveProject executeAnalyzer operation asynchronously."""
