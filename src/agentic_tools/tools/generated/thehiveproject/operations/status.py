...
class ThehiveprojectStatusToolInput(BaseModel):
    ...
    status: Optional[str] = Field(None, description="Choose from the list, or specify an ID using an <a href=\"https://docs.n8n.io/code-examples/expressions/\">expression</a>")
...
class ThehiveprojectStatusTool(BaseTool):
    name = "thehiveproject_status"
    description = "Tool for theHiveProject status operation - status operation"
    ...
    def _run(self, **kwargs):
        """Run the theHiveProject status operation."""
        ...
        if credentials:
            ...
            return f"Running theHiveProject status operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running theHiveProject status operation with default credentials and args: {kwargs}"
    
    async def _arun(self, **kwargs):
        """Run the theHiveProject status operation asynchronously."""
