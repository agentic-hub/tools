...
class ThehiveprojectSearchToolInput(BaseModel):
    ...
    all_tasks: Optional[bool] = Field(None, description="Whether to search in all tasks or only in selected task")
    ...
    search_in_knowledge_base: Optional[bool] = Field(None, description="Whether to search in knowledge base or only in the selected case")
    id: Optional[Dict[str, Any]] = Field(None, description="Alert")
    all_cases: Optional[bool] = Field(None, description="Whether to search in all cases or only in a selected case")
...
class ThehiveprojectSearchTool(BaseTool):
    name = "thehiveproject_search"
    description = "Tool for theHiveProject search operation - search operation"
    ...
    def _run(self, **kwargs):
        """Run the theHiveProject search operation."""
        ...
        if credentials:
            ...
            return f"Running theHiveProject search operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running theHiveProject search operation with default credentials and args: {kwargs}"
    
    async def _arun(self, **kwargs):
        """Run the theHiveProject search operation asynchronously."""
