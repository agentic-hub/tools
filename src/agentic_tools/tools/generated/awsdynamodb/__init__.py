# awsdynamodb toolkit
from agentic_tools.tools import BaseTool, BaseModel, Field
from agentic_tools.toolkit import AgenticHubToolkit
from typing import List, Optional, Dict, Any

def get_awsdynamodb_tools() -> List[BaseTool]:
    """Get all awsdynamodb tools."""
    from . import operations
    return operations.get_tools()

class AwsdynamodbCredentials(BaseModel):
    """Credentials for awsdynamodb authentication."""
    aws: Optional[Dict[str, Any]] = Field(None, description="aws")

class AwsDynamodbToolkit(AgenticHubToolkit):
    """Toolkit for interacting with awsdynamodb."""

    def __init__(self, credentials: Optional[AwsdynamodbCredentials] = None):
        """Initialize the awsdynamodb toolkit with optional credentials.

        Args:
            credentials: AwsdynamodbCredentials object containing authentication credentials
        """
        self.credentials = credentials

    def get_tools(self) -> List[BaseTool]:
        """Get all awsdynamodb tools with the configured credentials."""
        from . import operations
        return self.get_tools_from_operations(operations)
        # Apply credentials to each tool if provided
        if self.credentials:
            for tool in tools:
                # Set credentials on each tool instance
                tool.credentials = self.credentials
        return tools

    @staticmethod
    def get_default_tools() -> List[BaseTool]:
        """Get all awsdynamodb tools with default credentials."""
        return get_awsdynamodb_tools()
