...
class MailchimpGetallToolInput(BaseModel):
    ...
    group_category: Optional[str] = Field(None, description="Choose from the list, or specify an ID using an <a href=\"https://docs.n8n.io/code-examples/expressions/\">expression</a>")
...
class MailchimpGetallTool(BaseTool):
    ...
    def _run(self, **kwargs):
        ...
        if credentials:
            ...
            return f"Running mailchimp getAll operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running mailchimp getAll operation with default credentials and args: {kwargs}"
