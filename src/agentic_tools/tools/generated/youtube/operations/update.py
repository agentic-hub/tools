...
class YoutubeUpdateToolInput(BaseModel):
    ...
    playlist_id: Optional[str] = Field(None, description="The playlist's title")
...
class YoutubeUpdateTool(BaseTool):
    ...
    def _run(self, **kwargs):
        ...
        if credentials:
            ...
            return f"Running youTube update operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running youTube update operation with default credentials and args: {kwargs}"
