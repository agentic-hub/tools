...
class OpenaiCompleteToolInput(BaseModel):
    ...
    chat_model: Optional[str] = Field(None, description="The model which will generate the completion. <a href=\"https://beta.openai.com/docs/models/overview\">Learn more</a>.")
...
class OpenaiCompleteTool(BaseTool):
    ...
    def _run(self, **kwargs):
        ...
        if credentials:
            ...
            return f"Running openAi complete operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running openAi complete operation with default credentials and args: {kwargs}"
