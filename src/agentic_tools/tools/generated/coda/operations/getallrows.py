...
class CodaGetallrowsToolInput(BaseModel):
    ...
    table_id: Optional[str] = Field(None, description="The table to get the rows from. Choose from the list, or specify an ID using an <a href=\"https://docs.n8n.io/code-examples/expressions/\">expression</a>.")
...
class CodaGetallrowsTool(BaseTool):
    ...
    def _run(self, **kwargs):
        ...
        if credentials:
            ...
            return f"Running coda getAllRows operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running coda getAllRows operation with default credentials and args: {kwargs}"
