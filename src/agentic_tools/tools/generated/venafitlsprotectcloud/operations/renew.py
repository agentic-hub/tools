...
class VenafitlsprotectcloudRenewToolInput(BaseModel):
    ...
    existing_certificate_id: Optional[str] = Field(None, description="Existing Certificate ID")
...
class VenafitlsprotectcloudRenewTool(BaseTool):
    ...
    def _run(self, **kwargs):
        ...
        if credentials:
            ...
            return f"Running venafiTlsProtectCloud renew operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running venafiTlsProtectCloud renew operation with default credentials and args: {kwargs}"
