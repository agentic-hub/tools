...
class TrelloRemoveToolInput(BaseModel):
    ...
    id_member: Optional[str] = Field(None, description="The ID of the member to remove from the board")
...
class TrelloRemoveTool(BaseTool):
    ...
    def _run(self, **kwargs):
        ...
        if credentials:
            ...
            return f"Running trello remove operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running trello remove operation with default credentials and args: {kwargs}"
