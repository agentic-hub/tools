...
class LemlistUnsubscribeToolInput(BaseModel):
    ...
    campaign_id: Optional[str] = Field(None, description="ID of the campaign to unsubscribe the lead from. Choose from the list, or specify an ID using an <a href=\"https://docs.n8n.io/code-examples/expressions/\">expression</a>.")
...
class LemlistUnsubscribeTool(BaseTool):
    ...
    def _run(self, **kwargs):
        ...
        if credentials:
            ...
            return f"Running lemlist unsubscribe operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running lemlist unsubscribe operation with default credentials and args: {kwargs}"
