...
class KobotoolboxRetryoneToolInput(BaseModel):
    ...
    log_id: Optional[str] = Field(None, description="Hook log ID (starts with hl, e.g. hlSbGKaUKzTVNoWEVMYbLHe)")


class KobotoolboxRetryoneTool(BaseTool):
    name = "kobotoolbox_retryone"
    description = "Tool for koBoToolbox retryOne operation - retryOne operation"
    ...
    def _run(self, **kwargs):
        """Run the koBoToolbox retryOne operation."""
        ...
        if credentials:
            ...
            return f"Running koBoToolbox retryOne operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running koBoToolbox retryOne operation with default credentials and args: {kwargs}"
    
    async def _arun(self, **kwargs):
        """Run the koBoToolbox retryOne operation asynchronously."""
