# halopsa toolkit
from agentic_tools.tools import BaseTool, BaseModel, Field
from agentic_tools.toolkit import AgenticHubToolkit
from typing import List, Optional, Dict, Any

def get_halopsa_tools() -> List[BaseTool]:
    """Get all halopsa tools."""
    from . import operations
    return operations.get_tools()

class HalopsaCredentials(BaseModel):
    """Credentials for halopsa authentication."""
    halo_psa_api: Optional[Dict[str, Any]] = Field(None, description="haloPSAApi")

class HalopsaToolkit(AgenticHubToolkit):
    """Toolkit for interacting with halopsa."""

    def __init__(self, credentials: Optional[HalopsaCredentials] = None):
        """Initialize the halopsa toolkit with optional credentials.

        Args:
            credentials: HalopsaCredentials object containing authentication credentials
        """
        self.credentials = credentials

    def get_tools(self) -> List[BaseTool]:
        """Get all halopsa tools with the configured credentials."""
        from . import operations
        return self.get_tools_from_operations(operations)
        # Apply credentials to each tool if provided
        if self.credentials:
            for tool in tools:
                # Set credentials on each tool instance
                tool.credentials = self.credentials
        return tools

    @staticmethod
    def get_default_tools() -> List[BaseTool]:
        """Get all halopsa tools with default credentials."""
        return get_halopsa_tools()
