...
class BamboohrGetToolInput(BaseModel):
    ...
    report_id: Optional[str] = Field(None, description="ID of the report. You can get the report number by hovering over the report name on the reports page and grabbing the ID.")
...
class BamboohrGetTool(BaseTool):
    ...
    def _run(self, **kwargs):
        ...
        if credentials:
            ...
            return f"Running bambooHr get operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running bambooHr get operation with default credentials and args: {kwargs}"
