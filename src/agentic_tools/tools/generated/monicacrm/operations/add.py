...
class MonicacrmAddToolInput(BaseModel):
    ...
    conversationId: Optional[str] = Field(None, description="ID of the contact whose conversation")
    tagId: Optional[str] = Field(None, description="ID of the tag to delete")
    resource: Optional[str] = Field(None, description="Resource")
    additionalFields: Optional[Dict[str, Any]] = Field(None, description="Additional Fields")
    writtenByMe: Optional[str] = Field(None, description="Author of the message")
    contactId: Optional[str] = Field(None, description="ID of the contact to add a tag to")
    noteId: Optional[str] = Field(None, description="ID of the note to delete")
    writtenAt: Optional[str] = Field(None, description="Date when the message was written")
    title: Optional[str] = Field(None, description="Title of the journal entry - max 250 characters")


class MonicacrmAddTool(BaseTool):
    name = "monicacrm_add"
    description = "Tool for monicaCrm add operation - add operation"
    
    def _run(self, **kwargs):
        """Run the monicaCrm add operation."""
        # Implement the tool logic here
        return f"Running monicaCrm add operation with args: {kwargs}"
    
    async def _arun(self, **kwargs):
        """Run the monicaCrm add operation asynchronously."""
