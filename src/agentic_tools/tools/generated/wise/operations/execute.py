...
class WiseExecuteToolInput(BaseModel):
    ...
    transferId: Optional[str] = Field(None, description="ID of the transfer to execute")
    ...
    profileId: Optional[str] = Field(None, description="ID of the user profile to execute the transfer under. Choose from the list, or specify an ID using an <a href=\"https://docs.n8n.io/code-examples/expressions/\">expression</a>.")
    fileName: Optional[str] = Field(None, description="Name of the file that will be downloaded")


class WiseExecuteTool(BaseTool):
    name = "wise_execute"
    description = "Tool for wise execute operation - execute operation"
    
    def _run(self, **kwargs):
        """Run the wise execute operation."""
        # Implement the tool logic here
        return f"Running wise execute operation with args: {kwargs}"
    
    async def _arun(self, **kwargs):
        """Run the wise execute operation asynchronously."""
