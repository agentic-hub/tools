...
class PipedriveAddToolInput(BaseModel):
    ...
    item_price: Optional[float] = Field(None, description="Price at which to add or update this product in a deal")
    quantity: Optional[float] = Field(None, description="How many items of this product to add/update in a deal")
    ...
    deal_id: Optional[str] = Field(None, description="The ID of the deal to add a product to. Choose from the list, or specify an ID using an <a href=\"https://docs.n8n.io/code-examples/expressions/\">expression</a>.")
    ...
    product_id: Optional[str] = Field(None, description="The ID of the product to add to a deal. Choose from the list, or specify an ID using an <a href=\"https://docs.n8n.io/code-examples/expressions/\">expression</a>.")


class PipedriveAddTool(BaseTool):
    name = "pipedrive_add"
    description = "Tool for pipedrive add operation - add operation"
    ...
    def _run(self, **kwargs):
        """Run the pipedrive add operation."""
        ...
        if credentials:
            ...
            return f"Running pipedrive add operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running pipedrive add operation with default credentials and args: {kwargs}"
    
    async def _arun(self, **kwargs):
        """Run the pipedrive add operation asynchronously."""
