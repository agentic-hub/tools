...
class RedisPopToolInput(BaseModel):
    ...
    property_name: Optional[str] = Field(None, description="Optional name of the property to write received data to. Supports dot-notation. Example: \"data.person[0].name\".")
...
class RedisPopTool(BaseTool):
    ...
    def _run(self, **kwargs):
        ...
        if credentials:
            ...
            return f"Running redis pop operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running redis pop operation with default credentials and args: {kwargs}"
