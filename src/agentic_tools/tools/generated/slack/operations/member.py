...
class SlackMemberToolInput(BaseModel):
    ...
    channel_id: Optional[Dict[str, Any]] = Field(None, description="The Slack channel to get the members from")
    ...
    resolve_data: Optional[bool] = Field(None, description="Whether to resolve the data automatically. By default the response only contain the ID to resource.")
...
class SlackMemberTool(BaseTool):
    ...
    def _run(self, **kwargs):
        ...
        if credentials:
            ...
            return f"Running slack member operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running slack member operation with default credentials and args: {kwargs}"
