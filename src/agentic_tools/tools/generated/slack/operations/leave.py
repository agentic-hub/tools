...
class SlackLeaveToolInput(BaseModel):
    ...
    channel_id: Optional[Dict[str, Any]] = Field(None, description="The Slack channel to leave from")
...
class SlackLeaveTool(BaseTool):
    name = "slack_leave"
    description = "Tool for slack leave operation - leave operation"
    ...
    def _run(self, **kwargs):
        ...
        if credentials:
            ...
            return f"Running slack leave operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running slack leave operation with default credentials and args: {kwargs}"
    
    async def _arun(self, **kwargs):
        """Run the slack leave operation asynchronously."""
