...
class SlackJoinToolInput(BaseModel):
    ...
    channel_id: Optional[Dict[str, Any]] = Field(None, description="The Slack channel to join")
...
class SlackJoinTool(BaseTool):
    name = "slack_join"
    description = "Tool for slack join operation - join operation"
    ...
    def _run(self, **kwargs):
        ...
        if credentials:
            ...
            return f"Running slack join operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running slack join operation with default credentials and args: {kwargs}"
    
    async def _arun(self, **kwargs):
        """Run the slack join operation asynchronously."""
