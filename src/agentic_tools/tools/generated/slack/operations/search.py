...
class SlackSearchToolInput(BaseModel):
    # Allow users to provide their own credentials
    credentials: Optional[SlackCredentials] = Field(None, description="Custom credentials for authentication")
    update_fields: Optional[Dict[str, Any]] = Field(None, description="Update Fields")
    sort: Optional[str] = Field(None, description="How search results should be sorted. You can sort by.")
    ...
    query: Optional[str] = Field(None, description="The text to search for within messages")
...
class SlackSearchTool(BaseTool):
    name = "slack_search"
    description = "Tool for slack search operation - search operation"
    ...
    def _run(self, **kwargs):
        """Run the slack search operation."""
        ...
        if credentials:
            ...
            return f"Running slack search operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running slack search operation with default credentials and args: {kwargs}"
    
    async def _arun(self, **kwargs):
        """Run the slack search operation asynchronously."""
