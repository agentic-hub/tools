...
class SlackSetpurposeToolInput(BaseModel):
    ...
    channel_id: Optional[Dict[str, Any]] = Field(None, description="The Slack channel to set the purpose of")
...
class SlackSetpurposeTool(BaseTool):
    ...
    def _run(self, **kwargs):
        ...
        if credentials:
            ...
            return f"Running slack setPurpose operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running slack setPurpose operation with default credentials and args: {kwargs}"
