...
class SlackUpdateToolInput(BaseModel):
    ...
    channel_id: Optional[Dict[str, Any]] = Field(None, description="The Slack channel to update the message from")
    text: Optional[str] = Field(None, description="The message text to update. Supports <a href=\"https://api.slack.com/reference/surfaces/formatting/\">markdown</a> by default - this can be disabled in \"Options\".")
    ...
    ts: Optional[float] = Field(None, description="Timestamp of the message to update")
...
class SlackUpdateTool(BaseTool):
    name = "slack_update"
    description = "Tool for slack update operation - update operation"
    ...
    def _run(self, **kwargs):
        """Run the slack update operation."""
        ...
        if credentials:
            ...
            return f"Running slack update operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running slack update operation with default credentials and args: {kwargs}"
    
    async def _arun(self, **kwargs):
        """Run the slack update operation asynchronously."""
