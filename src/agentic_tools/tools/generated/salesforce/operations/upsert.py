...
class SalesforceUpsertToolInput(BaseModel):
    ...
    close_date: Optional[str] = Field(None, description="Required. Date when the opportunity is expected to close.")
    ...
    stage_name: Optional[str] = Field(None, description="Required. Date when the opportunity is expected to close. Choose from the list, or specify an ID using an <a href=\"https://docs.n8n.io/code-examples/expressions/\">expression</a>.")
...
class SalesforceUpsertTool(BaseTool):
    ...
    description = "Tool for salesforce upsert operation - upsert operation"
    ...
    def _run(self, **kwargs):
        """Run the salesforce upsert operation."""
        ...
        if credentials:
            ...
            return f"Running salesforce upsert operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running salesforce upsert operation with default credentials and args: {kwargs}"
    
    async def _arun(self, **kwargs):
        """Run the salesforce upsert operation asynchronously."""
