...
class SalesforceQueryToolInput(BaseModel):
    ...
    query: Optional[str] = Field(None, description="A SOQL query. An example query parameter string might look like: “SELECT+Name+FROM+MyObject”. If the SOQL query string is invalid, a MALFORMED_QUERY response is returned.")
...
class SalesforceQueryTool(BaseTool):
    ...
    description = "Tool for salesforce query operation - query operation"
    ...
    def _run(self, **kwargs):
        """Run the salesforce query operation."""
        ...
        if credentials:
            ...
            return f"Running salesforce query operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running salesforce query operation with default credentials and args: {kwargs}"
    
    async def _arun(self, **kwargs):
        """Run the salesforce query operation asynchronously."""
