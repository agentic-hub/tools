# pushover toolkit
from agentic_tools.tools import BaseTool, BaseModel, Field
from agentic_tools.toolkit import AgenticHubToolkit
from typing import List, Optional, Dict, Any

def get_pushover_tools() -> List[BaseTool]:
    """Get all pushover tools."""
    from . import operations
    return operations.get_tools()

class PushoverCredentials(BaseModel):
    """Credentials for pushover authentication."""
    pushover_api: Optional[Dict[str, Any]] = Field(None, description="pushoverApi")

class PushoverToolkit(AgenticHubToolkit):
    """Toolkit for interacting with pushover."""

    def __init__(self, credentials: Optional[PushoverCredentials] = None):
        """Initialize the pushover toolkit with optional credentials.

        Args:
            credentials: PushoverCredentials object containing authentication credentials
        """
        self.credentials = credentials

    def get_tools(self) -> List[BaseTool]:
        """Get all pushover tools with the configured credentials."""
        from . import operations
        return self.get_tools_from_operations(operations)
        # Apply credentials to each tool if provided
        if self.credentials:
            for tool in tools:
                # Set credentials on each tool instance
                tool.credentials = self.credentials
        return tools

    @staticmethod
    def get_default_tools() -> List[BaseTool]:
        """Get all pushover tools with default credentials."""
        return get_pushover_tools()
