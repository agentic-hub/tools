...
class MattermostDeleteToolInput(BaseModel):
    ...
    emoji_name: Optional[str] = Field(None, description="Name of the emoji to delete")
    ...
    user_id: Optional[str] = Field(None, description="ID of the user whose reaction to delete. Choose from the list, or specify an ID using an <a href=\"https://docs.n8n.io/code-examples/expressions/\">expression</a>.")
...
class MattermostDeleteTool(BaseTool):
    ...
    def _run(self, **kwargs):
        ...
        if credentials:
            ...
            return f"Running mattermost delete operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running mattermost delete operation with default credentials and args: {kwargs}"
