...
class MattermostAdduserToolInput(BaseModel):
    ...
    channel_id: Optional[str] = Field(None, description="The ID of the channel to invite user to. Choose from the list, or specify an ID using an <a href=\"https://docs.n8n.io/code-examples/expressions/\">expression</a>.")
...
class MattermostAdduserTool(BaseTool):
    ...
    def _run(self, **kwargs):
        ...
        if credentials:
            ...
            return f"Running mattermost addUser operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running mattermost addUser operation with default credentials and args: {kwargs}"
