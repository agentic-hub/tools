...
class OrbitUpsertToolInput(BaseModel):
    identityUi: Optional[Dict[str, Any]] = Field(None, description="The identity is used to find the member. If no member exists, a new member will be created and linked to the provided identity.")
...
class OrbitUpsertTool(BaseTool):
    name = "orbit_upsert"
    description = "Tool for orbit upsert operation - upsert operation"
    
    def _run(self, **kwargs):
        """Run the orbit upsert operation."""
        # Implement the tool logic here
        return f"Running orbit upsert operation with args: {kwargs}"
    
    async def _arun(self, **kwargs):
        """Run the orbit upsert operation asynchronously."""
