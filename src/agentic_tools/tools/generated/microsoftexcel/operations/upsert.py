...
class MicrosoftexcelUpsertToolInput(BaseModel):
...
class MicrosoftexcelUpsertTool(BaseTool):
    name = "microsoftexcel_upsert"
    description = "Tool for microsoftExcel upsert operation - upsert operation"
    ...
    def _run(self, **kwargs):
        """Run the microsoftExcel upsert operation."""
        ...
        if credentials:
            ...
            return f"Running microsoftExcel upsert operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running microsoftExcel upsert operation with default credentials and args: {kwargs}"
    
    async def _arun(self, **kwargs):
        """Run the microsoftExcel upsert operation asynchronously."""
