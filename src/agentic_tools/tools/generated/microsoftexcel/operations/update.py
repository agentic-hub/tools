...
class MicrosoftexcelUpdateToolInput(BaseModel):
    # Allow users to provide their own credentials
    credentials: Optional[MicrosoftexcelCredentials] = Field(None, description="Custom credentials for authentication")
    data: Optional[str] = Field(None, description="Raw values for the specified range as array of string arrays in JSON format. Should match the specified range: one array item for each row.")
    ...
    range: Optional[str] = Field(None, description="The sheet range to read the data from specified using a A1-style notation, has to be specific e.g A1:B5, generic ranges like A:B are not supported. Leave blank to use whole used range in the sheet.")
...
class MicrosoftexcelUpdateTool(BaseTool):
    name = "microsoftexcel_update"
    description = "Tool for microsoftExcel update operation - update operation"
    ...
    def _run(self, **kwargs):
        """Run the microsoftExcel update operation."""
        ...
        if credentials:
            ...
            return f"Running microsoftExcel update operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running microsoftExcel update operation with default credentials and args: {kwargs}"
    
    async def _arun(self, **kwargs):
        """Run the microsoftExcel update operation asynchronously."""
