...
class PagerdutyGetToolInput(BaseModel):
    ...
    log_entry_id: Optional[str] = Field(None, description="Unique identifier for the log entry")
...
class PagerdutyGetTool(BaseTool):
    ...
    def _run(self, **kwargs):
        ...
        if credentials:
            ...
            return f"Running pagerDuty get operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running pagerDuty get operation with default credentials and args: {kwargs}"
