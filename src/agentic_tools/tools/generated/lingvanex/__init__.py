# lingvanex toolkit
from agentic_tools.tools import BaseTool, BaseModel, Field
from agentic_tools.toolkit import AgenticHubToolkit
from typing import List, Optional, Dict, Any

def get_lingvanex_tools() -> List[BaseTool]:
    """Get all lingvanex tools."""
    from . import operations
    return operations.get_tools()

class LingvanexCredentials(BaseModel):
    """Credentials for lingvanex authentication."""
    lingva_nex_api: Optional[Dict[str, Any]] = Field(None, description="lingvaNexApi")

class LingvanexToolkit(AgenticHubToolkit):
    """Toolkit for interacting with lingvanex."""

    def __init__(self, credentials: Optional[LingvanexCredentials] = None):
        """Initialize the lingvanex toolkit with optional credentials.

        Args:
            credentials: LingvanexCredentials object containing authentication credentials
        """
        self.credentials = credentials

    def get_tools(self) -> List[BaseTool]:
        """Get all lingvanex tools with the configured credentials."""
        from . import operations
        return self.get_tools_from_operations(operations)
        # Apply credentials to each tool if provided
        if self.credentials:
            for tool in tools:
                # Set credentials on each tool instance
                tool.credentials = self.credentials
        return tools

    @staticmethod
    def get_default_tools() -> List[BaseTool]:
        """Get all lingvanex tools with default credentials."""
        return get_lingvanex_tools()
