# thehive toolkit
from agentic_tools.tools import BaseTool, BaseModel, Field
from agentic_tools.toolkit import AgenticHubToolkit
from typing import List, Optional, Dict, Any

def get_thehive_tools() -> List[BaseTool]:
    """Get all thehive tools."""
    from . import operations
    return operations.get_tools()

class ThehiveCredentials(BaseModel):
    """Credentials for thehive authentication."""
    the_hive_api: Optional[Dict[str, Any]] = Field(None, description="theHiveApi")

class ThehiveToolkit(AgenticHubToolkit):
    """Toolkit for interacting with thehive."""

    def __init__(self, credentials: Optional[ThehiveCredentials] = None):
        """Initialize the thehive toolkit with optional credentials.

        Args:
            credentials: ThehiveCredentials object containing authentication credentials
        """
        self.credentials = credentials

    def get_tools(self) -> List[BaseTool]:
        """Get all thehive tools with the configured credentials."""
        from . import operations
        return self.get_tools_from_operations(operations)
        # Apply credentials to each tool if provided
        if self.credentials:
            for tool in tools:
                # Set credentials on each tool instance
                tool.credentials = self.credentials
        return tools

    @staticmethod
    def get_default_tools() -> List[BaseTool]:
        """Get all thehive tools with default credentials."""
        return get_thehive_tools()
