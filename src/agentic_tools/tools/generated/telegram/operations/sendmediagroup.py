...
class TelegramSendmediagroupToolInput(BaseModel):
    ...
    media: Optional[Dict[str, Any]] = Field(None, description="The media to add")
    force_reply: Optional[Dict[str, Any]] = Field(None, description="Force Reply")


class TelegramSendmediagroupTool(BaseTool):
    name = "telegram_sendmediagroup"
    description = "Tool for telegram sendMediaGroup operation - sendMediaGroup operation"
    ...
    def _run(self, **kwargs):
        """Run the telegram sendMediaGroup operation."""
        ...
        if credentials:
            ...
            return f"Running telegram sendMediaGroup operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running telegram sendMediaGroup operation with default credentials and args: {kwargs}"
    
    async def _arun(self, **kwargs):
        """Run the telegram sendMediaGroup operation asynchronously."""
