...
class TelegramSendstickerToolInput(BaseModel):
    ...
    file: Optional[str] = Field(None, description="Sticker to send. Pass a file_id to send a file that exists on the Telegram servers (recommended), an HTTP URL for Telegram to get a .webp file from the Internet.")
...
class TelegramSendstickerTool(BaseTool):
    name = "telegram_sendsticker"
    description = "Tool for telegram sendSticker operation - sendSticker operation"
    ...
    def _run(self, **kwargs):
        """Run the telegram sendSticker operation."""
        ...
        if credentials:
            ...
            return f"Running telegram sendSticker operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running telegram sendSticker operation with default credentials and args: {kwargs}"
    
    async def _arun(self, **kwargs):
        """Run the telegram sendSticker operation asynchronously."""
