...
class TelegramMemberToolInput(BaseModel):
    ...
    user_id: Optional[str] = Field(None, description="Unique identifier of the target user")
...
class TelegramMemberTool(BaseTool):
    ...
    def _run(self, **kwargs):
        ...
        if credentials:
            ...
            return f"Running telegram member operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running telegram member operation with default credentials and args: {kwargs}"
