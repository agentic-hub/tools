# copper toolkit
from agentic_tools.tools import BaseTool, BaseModel, Field
from agentic_tools.toolkit import AgenticHubToolkit
from typing import List, Optional, Dict, Any

def get_copper_tools() -> List[BaseTool]:
    """Get all copper tools."""
    from . import operations
    return operations.get_tools()

class CopperCredentials(BaseModel):
    """Credentials for copper authentication."""
    copper_api: Optional[Dict[str, Any]] = Field(None, description="copperApi")

class CopperToolkit(AgenticHubToolkit):
    """Toolkit for interacting with copper."""

    def __init__(self, credentials: Optional[CopperCredentials] = None):
        """Initialize the copper toolkit with optional credentials.

        Args:
            credentials: CopperCredentials object containing authentication credentials
        """
        self.credentials = credentials

    def get_tools(self) -> List[BaseTool]:
        """Get all copper tools with the configured credentials."""
        from . import operations
        return self.get_tools_from_operations(operations)
        # Apply credentials to each tool if provided
        if self.credentials:
            for tool in tools:
                # Set credentials on each tool instance
                tool.credentials = self.credentials
        return tools

    @staticmethod
    def get_default_tools() -> List[BaseTool]:
        """Get all copper tools with default credentials."""
        return get_copper_tools()
