# venafitlsprotectdatacenter toolkit
from agentic_tools.tools import BaseTool, BaseModel, Field
from agentic_tools.toolkit import AgenticHubToolkit
from typing import List, Optional, Dict, Any

def get_venafitlsprotectdatacenter_tools() -> List[BaseTool]:
    """Get all venafitlsprotectdatacenter tools."""
    from . import operations
    return operations.get_tools()

class VenafitlsprotectdatacenterCredentials(BaseModel):
    """Credentials for venafitlsprotectdatacenter authentication."""
    venafi_tls_protect_datacenter_api: Optional[Dict[str, Any]] = Field(None, description="venafiTlsProtectDatacenterApi")

class VenafitlsprotectdatacenterToolkit(AgenticHubToolkit):
    """Toolkit for interacting with venafitlsprotectdatacenter."""

    def __init__(self, credentials: Optional[VenafitlsprotectdatacenterCredentials] = None):
        """Initialize the venafitlsprotectdatacenter toolkit with optional credentials.

        Args:
            credentials: VenafitlsprotectdatacenterCredentials object containing authentication credentials
        """
        self.credentials = credentials

    def get_tools(self) -> List[BaseTool]:
        """Get all venafitlsprotectdatacenter tools with the configured credentials."""
        from . import operations
        return self.get_tools_from_operations(operations)
        # Apply credentials to each tool if provided
        if self.credentials:
            for tool in tools:
                # Set credentials on each tool instance
                tool.credentials = self.credentials
        return tools

    @staticmethod
    def get_default_tools() -> List[BaseTool]:
        """Get all venafitlsprotectdatacenter tools with default credentials."""
        return get_venafitlsprotectdatacenter_tools()
