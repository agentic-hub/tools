...
class QuickbaseRunToolInput(BaseModel):
...
class QuickbaseRunTool(BaseTool):
    name = "quickbase_run"
    description = "Tool for quickbase run operation - run operation"
    ...
    def _run(self, **kwargs):
        """Run the quickbase run operation."""
        ...
        if credentials:
            ...
            return f"Running quickbase run operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running quickbase run operation with default credentials and args: {kwargs}"
    
    async def _arun(self, **kwargs):
        """Run the quickbase run operation asynchronously."""
