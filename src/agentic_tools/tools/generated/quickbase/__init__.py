# quickbase toolkit
from agentic_tools.tools import BaseTool, BaseModel, Field
from agentic_tools.toolkit import AgenticHubToolkit
from typing import List, Optional, Dict, Any

def get_quickbase_tools() -> List[BaseTool]:
    """Get all quickbase tools."""
    from . import operations
    return operations.get_tools()

class QuickbaseCredentials(BaseModel):
    """Credentials for quickbase authentication."""
    quickbase_api: Optional[Dict[str, Any]] = Field(None, description="quickbaseApi")

class QuickbaseToolkit(AgenticHubToolkit):
    """Toolkit for interacting with quickbase."""

    def __init__(self, credentials: Optional[QuickbaseCredentials] = None):
        """Initialize the quickbase toolkit with optional credentials.

        Args:
            credentials: QuickbaseCredentials object containing authentication credentials
        """
        self.credentials = credentials

    def get_tools(self) -> List[BaseTool]:
        """Get all quickbase tools with the configured credentials."""
        from . import operations
        return self.get_tools_from_operations(operations)
        # Apply credentials to each tool if provided
        if self.credentials:
            for tool in tools:
                # Set credentials on each tool instance
                tool.credentials = self.credentials
        return tools

    @staticmethod
    def get_default_tools() -> List[BaseTool]:
        """Get all quickbase tools with default credentials."""
        return get_quickbase_tools()
