...
class Scade-toolsUpdateToolInput(BaseModel):
    ...
    workflow_object: Optional[str] = Field(None, description="A valid JSON object with required fields: 'name', 'nodes', 'connections' and 'settings'. More information can be found in the <a href=\"https://docs.n8n.io/api/api-reference/#tag/Workflow/paths/~1workflows~1%7Bid%7D/put\">documentation</a>.")
...
class Scade-toolsUpdateTool(BaseTool):
    name = "scade-tools_update"
    description = "Tool for scade-tools update operation - update operation"
    ...
    def _run(self, **kwargs):
        """Run the scade-tools update operation."""
        ...
        if credentials:
            ...
            return f"Running scade-tools update operation with custom credentials {safe_credentials} and args: {kwargs}"
        else:
            return f"Running scade-tools update operation with default credentials and args: {kwargs}"
    
    async def _arun(self, **kwargs):
        """Run the scade-tools update operation asynchronously."""
