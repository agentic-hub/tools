# contentful toolkit
from agentic_tools.tools import BaseTool, BaseModel, Field
from agentic_tools.toolkit import AgenticHubToolkit
from typing import List, Optional, Dict, Any

def get_contentful_tools() -> List[BaseTool]:
    """Get all contentful tools."""
    from . import operations
    return operations.get_tools()

class ContentfulCredentials(BaseModel):
    """Credentials for contentful authentication."""
    contentful_api: Optional[Dict[str, Any]] = Field(None, description="contentfulApi")

class ContentfulToolkit(AgenticHubToolkit):
    """Toolkit for interacting with contentful."""

    def __init__(self, credentials: Optional[ContentfulCredentials] = None):
        """Initialize the contentful toolkit with optional credentials.

        Args:
            credentials: ContentfulCredentials object containing authentication credentials
        """
        self.credentials = credentials

    def get_tools(self) -> List[BaseTool]:
        """Get all contentful tools with the configured credentials."""
        from . import operations
        return self.get_tools_from_operations(operations)
        # Apply credentials to each tool if provided
        if self.credentials:
            for tool in tools:
                # Set credentials on each tool instance
                tool.credentials = self.credentials
        return tools

    @staticmethod
    def get_default_tools() -> List[BaseTool]:
        """Get all contentful tools with default credentials."""
        return get_contentful_tools()
