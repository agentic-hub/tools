# beeminder toolkit
from agentic_tools.tools import BaseTool, BaseModel, Field
from agentic_tools.toolkit import AgenticHubToolkit
from typing import List, Optional, Dict, Any

def get_beeminder_tools() -> List[BaseTool]:
    """Get all beeminder tools."""
    from . import operations
    return operations.get_tools()

class BeeminderCredentials(BaseModel):
    """Credentials for beeminder authentication."""
    beeminder_api: Optional[Dict[str, Any]] = Field(None, description="beeminderApi")

class BeeminderToolkit(AgenticHubToolkit):
    """Toolkit for interacting with beeminder."""

    def __init__(self, credentials: Optional[BeeminderCredentials] = None):
        """Initialize the beeminder toolkit with optional credentials.

        Args:
            credentials: BeeminderCredentials object containing authentication credentials
        """
        self.credentials = credentials

    def get_tools(self) -> List[BaseTool]:
        """Get all beeminder tools with the configured credentials."""
        from . import operations
        return self.get_tools_from_operations(operations)
        # Apply credentials to each tool if provided
        if self.credentials:
            for tool in tools:
                # Set credentials on each tool instance
                tool.credentials = self.credentials
        return tools

    @staticmethod
    def get_default_tools() -> List[BaseTool]:
        """Get all beeminder tools with default credentials."""
        return get_beeminder_tools()
