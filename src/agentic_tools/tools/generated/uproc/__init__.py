# uproc toolkit
from agentic_tools.tools import BaseTool, BaseModel, Field
from agentic_tools.toolkit import AgenticHubToolkit
from typing import List, Optional, Dict, Any

def get_uproc_tools() -> List[BaseTool]:
    """Get all uproc tools."""
    from . import operations
    return operations.get_tools()

class UprocCredentials(BaseModel):
    """Credentials for uproc authentication."""
    uproc_api: Optional[Dict[str, Any]] = Field(None, description="uprocApi")

class UprocToolkit(AgenticHubToolkit):
    """Toolkit for interacting with uproc."""

    def __init__(self, credentials: Optional[UprocCredentials] = None):
        """Initialize the uproc toolkit with optional credentials.

        Args:
            credentials: UprocCredentials object containing authentication credentials
        """
        self.credentials = credentials

    def get_tools(self) -> List[BaseTool]:
        """Get all uproc tools with the configured credentials."""
        from . import operations
        return self.get_tools_from_operations(operations)
        # Apply credentials to each tool if provided
        if self.credentials:
            for tool in tools:
                # Set credentials on each tool instance
                tool.credentials = self.credentials
        return tools

    @staticmethod
    def get_default_tools() -> List[BaseTool]:
        """Get all uproc tools with default credentials."""
        return get_uproc_tools()
